# STATUS.md (2026-04-26 v8)

end-to-end 파이프라인 + 동적 vocab + 시세 알고리즘 + accessory 분리 + new_price floor + tenacity retry + 카테고리 자동 매핑 + 운영 로그(crawl_runs) + 이상 감지 알림 + launchd 스케줄러 + 시세 시계열(7d/28d 트렌드) + 매칭 회귀 fixture 운영 중. 다음 개선 항목은 [`NEXT.md`](NEXT.md) 참고.
...
├── run_all.py                    # 9개 × 5개 + discovery + aggregate (오케스트레이터)
...
tests/                            # 51 passed (fixture 기반, 네트워크 미의존)
```
